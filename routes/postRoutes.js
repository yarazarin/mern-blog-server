...
router.get("/stats", auth, async (req, res) => {
  try {
    ...
    const totalViews = await Post.aggregate([{ $group: { _id: null, total: { $sum: "$viewCount" } } }]);
    const views = totalViews.length > 0 ? totalViews[0].total : 0;
    const topPosts = await Post.find().sort({ viewCount: -1 }).limit(5).select('title viewCount');
    res.json({
      totalPosts,
      totalViews: views,
      topPosts
