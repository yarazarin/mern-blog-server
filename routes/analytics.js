...
router.get("/views", auth, async (req, res) => {
    try {
        ...
        const cleanAdminIp = adminIp
            ? adminIp.replace(/^::ffff:/, "")
            : null;
        let match = cleanAdminIp
            ? { ip: { $ne: cleanAdminIp } }
            : {};
...
router.get("/recent", auth, async (req, res) => {
    try {
        ...
        const visits = await Visit.find(query)
            .sort({ date: -1 })
            .limit(limit)
            .select("-_id ip country city date path");
        res.json(visits);
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
...
router.get("/devices", auth, async (req, res) => {
    try {
        ...
        const visits = await Visit.find(match).select(
            "userAgent"
        );
...
router.get("/all-visits", auth, async (req, res) => {
    try {
        ...
        const visits = await Visit.find(query)
            ...
            .limit(limit)
            .select(
                "-_id ip country region city date path userAgent referrer"
            );
